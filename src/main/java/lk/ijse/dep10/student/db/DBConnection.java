...
public class DBConnection {
    ...
    public DBConnection() {
        try {
            ...
            String password = properties.getProperty("mysql.password", "Soiya123");

            String url = "jdbc:mysql://" + host+":"+port+"/"+database+"?creatIfNotExist=true?allowMultiQueries=true";
            connection= DriverManager.getConnection(url, username, password);

        } catch (Exception e) {
            e.printStackTrace();
            new Alert(Alert.AlertType.ERROR,
                    "Failed to obtain the database connection").showAndWait();
            System.exit(1);
